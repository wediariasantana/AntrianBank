        ...
        <div class="col s12">
            <div class="card" style="background:#f9f9f9;box-shadow:none">
                ...
                @foreach($departments as $department)
                    <span class="btn btn-large btn-queue waves-effect waves-light" onclick="queue_dept({{ $department->id }})">{{ $department->name }}</span><br>
                @endforeach
            </div>
...
@section('print')
    @if(session()->has('department_name'))
        ...
        <div id="printarea" style="line-height:1.25">
            ...
            <span style="font-size:20px">Total Antrian( {{session()->get('total')}} ) Antrian Menunggu : {{ session()->get('total')-1 }}</span><br>
